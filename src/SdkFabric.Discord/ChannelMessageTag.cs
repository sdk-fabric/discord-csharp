...
public class ChannelMessageTag : TagAbstract {
    public ChannelMessageTag(RestClient httpClient, Parser parser): base(httpClient, parser)
    {
    }
     ...
     */
    public async Task<List<Message>> GetAll(string channelId, string around, string before, string after, int limit)
    {
    ...
    public async Task<Message> Get(string channelId, string messageId)
    {
        ...
        if (response.IsSuccessful)
        {
            return this.Parser.Parse<Message>(response.Content);
        }
